...
import cv2
gray_img = cv2.imread(r"G:/ACV CA 1/heart png.png", cv2.IMREAD_GRAYSCALE)
...
import cv2
img = cv2.imread(r"G:/ACV CA 1/heart png.png")
cv2.imwrite('G:/ACV CA 1/heart (Grayscale).png', img, [cv2.IMWRITE_PNG_COMPRESSION])

import cv2
print([x for x in dir(cv2) if x.startswith('COLOR_')]) # RGB, CMYK, YUV, HSV - Hue, Saturation,

import cv2
img = cv2.imread(r"G:/ACV CA 1/Y channel brain.png", cv2.IMREAD_COLOR)
...
import cv2
import numpy as np
img = cv2.imread(r"G:/ACV CA 1/Translation brain grey.png")
num_rows, num_cols = img.shape[:2]
translation_matrix = np.float32([ [1,0,70], [0,1,110] ])
img_translation = cv2.warpAffine(img, translation_matrix, (num_cols + 70, num_rows + 110))
...
import cv2
import numpy as np
img = cv2.imread(r"C:\Users\CSE-14\Downloads\Datasets (3)\mandrill.tif")
num_rows, num_cols = img.shape[:2]
translation_matrix = np.float32([ [1,0,70], [0,1,110] ])
...
img = cv2.imread(r"G:/ACV CA 1/V channel brain.png")
num_rows, num_cols = img.shape[:2]
rotation_matrix = cv2.getRotationMatrix2D((num_cols/2, num_rows/2), 80, 0.9)
...
import cv2 
import numpy as np
img = cv2.imread('G:/ACV CA 1/GRB original img.png')
rows, cols = img.shape[:2]
...
import numpy as np
import math
img = cv2.imread('/Users/apple/Documents/Deep Learning using Keras/mandrill.tif', cv2.IMREAD_GRAYSCALE)
rows, cols = img.shape
img_output = np.zeros(img.shape, dtype=img.dtype)
for i in range(rows):
    for j in range(cols):
...
import cv2
import numpy as np
img = cv2.imread(r"G:/ACV CA 1/Motion Blur kidney anatomy.png")
cv2.imshow('Original', img)
